...
void CRAFExtractorDlg::addRootDirectories()
{
	unordered_set<string> dirs;

	for (auto& i : files)
	{
		for (auto& curFile : i.fileEntries)
		{
			string relativePath = curFile.pathString;
			
			size_t endPos = relativePath.find_first_of("/", 0);
			string rootDirectory = relativePath.substr(0,endPos);

			dirs.insert(rootDirectory);
		}
	}
...
bool CRAFExtractorDlg::findFiles(wstring path, vector<wstring>& files)
{
	wstring mask = _T("*");
		...
		do
		{
			if (wcscmp(findData.cFileName, _T(".")) != 0 && wcscmp(findData.cFileName, _T("..")) != 0)
			{
				...
				else
				{
					...
					{
						files.push_back(path + _T("\\") + findData.cFileName);
					}
				}
			}
...
void CRAFExtractorDlg::openDir()
{
	...
	try
	{
		...
		{
			RAFReader inRAFReader;
			inRAFReader.readFile(curArchive.c_str());
			files.push_back(inRAFReader);
		}
...
void CRAFExtractorDlg::dynamicallyLoadChildren(HTREEITEM hParent)
{
	...
	{
		for (auto& curFile : files.at(i).fileEntries)
		{
			string relativePath = curFile.pathString;
	...
	for (auto& curFile : uniqueFiles)
	{
		...
		tvItem.item.lParam = reinterpret_cast<LPARAM>(&treePointers.back());
		HTREEITEM hNew = m_FileTreeView.InsertItem(&tvItem);
	...
	if (m_FilesRadio.GetCheck())
	{
		...
		for (size_t i = 0; i < archives.size(); ++i)
		{
			for (auto& curFile : files.at(i).fileEntries)
			{
				...
				string fileName = relativePath.substr(fileIndex);
...
void CRAFExtractorDlg::extractItem(HTREEITEM hItem)
{
	...
	if (m_FileTreeView.ItemHasChildren(hItem))
	{
		...
		treeItemDirs = treeItemDirs.substr(0, fileIndex);

		for (size_t it = 0; it < archives.size(); ++it)
		{
			for (auto& curFile : files.at(it).fileEntries)
			{
				...
				try
				{
					RAFDataReader inRAFDataReader(curFile, archives.at(it));
