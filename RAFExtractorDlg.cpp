...
void CRAFExtractorDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_RADIODIRS, m_DirsRadio);
	DDX_Control(pDX, IDC_BUTTONSEARCH, m_SearchButton);
	DDX_Control(pDX, IDC_BUTTONCLEAR, m_ClearButton);
	DDX_Control(pDX, IDC_EXTRACT, m_ExtractButton);
...
BOOL CRAFExtractorDlg::OnInitDialog()
{
	...
	m_FileTreeView.GetWindowRect(&treeRect);
	ScreenToClient(&treeRect);

	m_ProgressInfo.GetWindowRect(&progressInfoRect);
	ScreenToClient(&progressInfoRect);

	m_ProgressBar.GetWindowRect(&progressBarRect);
	ScreenToClient(&progressBarRect);

	m_FilesRadio.GetWindowRect(&filesRadioRect);
	ScreenToClient(&filesRadioRect);

	m_DirsRadio.GetWindowRect(&dirsRadioRect);
	ScreenToClient(&dirsRadioRect);

	m_SearchEdit.GetWindowRect(&searchEditRect);
	ScreenToClient(&searchEditRect);

	m_SearchButton.GetWindowRect(&searchButtonRect);
	ScreenToClient(&searchButtonRect);

	m_ClearButton.GetWindowRect(&clearButtonRect);
	ScreenToClient(&clearButtonRect);

	m_ExtractButton.GetWindowRect(&extractButtonRect);
	ScreenToClient(&extractButtonRect);

	m_ExitButton.GetWindowRect(&exitButtonRect);
	ScreenToClient(&exitButtonRect);
...
void CRAFExtractorDlg::OnBnClickedOpenfile()
{
	...
	m_TreeInfo.SetWindowTextW(_T("Viewing 1 archive."));
}
...
void CRAFExtractorDlg::openDir()
{
	...
	try
	{
		...
		CString treeInfo;
		treeInfo.Format(_T("Viewing %d archives."), archives.size());
		m_TreeInfo.SetWindowTextW(treeInfo);
	}
...
void CRAFExtractorDlg::search()
{
	...
	if (m_FilesRadio.GetCheck())
	{
		...
		CString treeInfo;
		treeInfo.Format(_T("Viewing %d files containing \"%s\""), uniqueFiles.size(), searchText);
		m_TreeInfo.SetWindowTextW(treeInfo);
	}
	...
	else
	{
		...
		CString treeInfo;
		treeInfo.Format(_T("Viewing %d directories containing \"%s\""), uniqueDirs.size(), searchText);
		m_TreeInfo.SetWindowTextW(treeInfo);
	}
}
...
void CRAFExtractorDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	int xOffset = cx - windowRect.right;
	int yOffset = cy - windowRect.bottom;
	...
	CRect newRect;
	newRect.left = treeRect.left;
	newRect.top = treeRect.top;
	newRect.right = treeRect.right + xOffset;
	newRect.bottom = treeRect.bottom + yOffset;

	m_FileTreeView.MoveWindow(&newRect);

	newRect.left = progressInfoRect.left;
	newRect.top = progressInfoRect.top + yOffset;
	newRect.right = progressInfoRect.right + xOffset;
	newRect.bottom = progressInfoRect.bottom + yOffset;

	m_ProgressInfo.MoveWindow(&newRect);

	newRect.left = progressBarRect.left;
	newRect.top = progressBarRect.top + yOffset;
	newRect.right = progressBarRect.right + xOffset;
	newRect.bottom = progressBarRect.bottom + yOffset;

	m_ProgressBar.MoveWindow(&newRect);

	newRect.left = filesRadioRect.left + xOffset;
	newRect.top = filesRadioRect.top;
	newRect.right = filesRadioRect.right + xOffset;
	newRect.bottom = filesRadioRect.bottom;

	m_FilesRadio.MoveWindow(&newRect);

	newRect.left = dirsRadioRect.left + xOffset;
	newRect.top = dirsRadioRect.top;
	newRect.right = dirsRadioRect.right + xOffset;
	newRect.bottom = dirsRadioRect.bottom;

	m_DirsRadio.MoveWindow(&newRect);

	newRect.left = searchEditRect.left + xOffset;
	newRect.top = searchEditRect.top;
	newRect.right = searchEditRect.right + xOffset;
	newRect.bottom = searchEditRect.bottom;

	m_SearchEdit.MoveWindow(&newRect);

	newRect.left = searchButtonRect.left + xOffset;
	newRect.top = searchButtonRect.top;
	newRect.right = searchButtonRect.right + xOffset;
	newRect.bottom = searchButtonRect.bottom;

	m_SearchButton.MoveWindow(&newRect);

	newRect.left = clearButtonRect.left + xOffset;
	newRect.top = clearButtonRect.top;
	newRect.right = clearButtonRect.right + xOffset;
	newRect.bottom = clearButtonRect.bottom;

	m_ClearButton.MoveWindow(&newRect);

	newRect.left = extractButtonRect.left + xOffset;
	newRect.top = extractButtonRect.top + yOffset;
	newRect.right = extractButtonRect.right + xOffset;
	newRect.bottom = extractButtonRect.bottom + yOffset;

	m_ExtractButton.MoveWindow(&newRect);

	newRect.left = exitButtonRect.left + xOffset;
	newRect.top = exitButtonRect.top + yOffset;
	newRect.right = exitButtonRect.right + xOffset;
	newRect.bottom = exitButtonRect.bottom + yOffset;

	m_ExitButton.MoveWindow(&newRect);
}
